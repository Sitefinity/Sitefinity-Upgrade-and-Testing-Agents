 ...
 */
function deleteSpecFiles(dirPath: string, result: CleanResult): void {
  if (!existsSync(dirPath)) return;
...
export async function cleanEnvironmentHandler(): Promise<{
  ...
  result.summary = result.success
    ? `Environment cleared. ${result.deleted.length} spec file(s) deleted, ${result.restored.length} file(s) restored from defaults, ${result.cleared.length} director(ies) cleared.`
    : `Clean completed with ${result.errors.length} error(s). Review the errors array for details.`;
