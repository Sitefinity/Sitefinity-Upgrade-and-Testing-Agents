...
1. Call `scaffold_test_directory` with `targetPath: {SourceProjectPath}`
   - Creates `sitefinity-tests/` with playwright.config.ts, package.json, .gitignore, README.md, tests, settings, and snapshots
   - Copies all tests from upgrade-and-testing/tests/ to sitefinity-tests/tests/
   - Copies all snapshots from upgrade-and-testing/snapshots/ to sitefinity-tests/snapshots/

2. **Verify snapshots were copied**: Check that `{SourceProjectPath}/sitefinity-tests/snapshots/` contains snapshot images
   - If snapshots are missing, investigate why scaffold didn't copy them
   - Snapshots should have been generated by sf-test-healer in upgrade-and-testing
   - If missing in upgrade-and-testing, you may need to copy from test-artifacts or regenerate
   - **Do not proceed without snapshots** - VRT tests require pre-upgrade baselines. 
   - If snapshots are missing and actions need to be taken to regenerate / copy them - advise the user about that and then proceed to provisioning them.
...
✅ Tests copied from upgrade-and-testing
✅ Snapshots copied from upgrade-and-testing (pre-upgrade baselines)
